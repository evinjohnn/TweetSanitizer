...
async function addFlagToUsername(usernameElement, screenName) {
  ...
  try {
    ...
    // Try to trigger onboarding (if applicable)
    maybeStartOnboarding(placeholderPill);
...
// --- ONBOARDING FLOW (v4.0.4) ---
const ONBOARDING_KEY_V404 = 'ts_onboarding_v404_done';
let onboardingCheckInProgress = false; // Lock to ensure only top-most (first) pill triggers it

async function isOnboardingCompleted() {
  const result = await chrome.storage.local.get([ONBOARDING_KEY_V404, 'debug_mode_enabled']);
  const isDone = !!result[ONBOARDING_KEY_V404];
  const isDebug = !!result.debug_mode_enabled;

  // If Debug Mode is ON, we treat it as NOT completed (so it shows again)
  if (isDebug) return false;

  return isDone;
}

async function maybeStartOnboarding(pillEl) {
  // Static check: Only run once per page load to target the first/top pill
  if (onboardingCheckInProgress) return;
  onboardingCheckInProgress = true;

  // Check if onboarding is already showing
  if (document.querySelector('.ts-coachmark')) return;

  // Utilize the new helper
  const isCompleted = await isOnboardingCompleted();
  if (isCompleted) return;

  // Add highlight immediately to catch attention
  pillEl.classList.add('ts-onboarding-highlight');

  // Wait 1s before showing the first coachmark (User requested 1s)
  setTimeout(() => {
    // Ensure pill is still in DOM
    if (!document.contains(pillEl)) return;
    showOnboardingStep1(pillEl);
  }, 1000);
}

// Helper: Create Coachmark
function createCoachmark({ target, text, placement = 'bottom', onNext, onSkip }) {
  cleanupCoachmark();

  const rect = target.getBoundingClientRect();
  const coachmark = document.createElement('div');
  coachmark.className = 'ts-coachmark';

  coachmark.innerHTML = `
    <div class="ts-coachmark-content">
      <div style="margin-bottom: 12px; line-height: 1.4;">${text}</div>
      <div class="ts-coachmark-actions">
        <button data-action="skip">Skip</button>
        <button data-action="next">Next</button>
      </div>
    </div>
    <div class="ts-coachmark-arrow"></div>
  `;

  document.body.appendChild(coachmark);

  // Positioning
  const arrow = coachmark.querySelector('.ts-coachmark-arrow');
  ...
  if (placement === 'bottom') {
    ...
    // Arrow top
    arrow.style.cssText = 'top: -6px; left: 50%; margin-left: -6px; border-left: 6px solid transparent; border-right: 6px solid transparent; border-bottom: 6px solid #0f1419;';
  } else if (placement === 'top') {
    top = rect.top - cardRect.height - 12;
    left = rect.left + (rect.width / 2) - (cardRect.width / 2);
    // Arrow bottom
    arrow.style.cssText = 'bottom: -6px; left: 50%; margin-left: -6px; border-left: 6px solid transparent; border-right: 6px solid transparent; border-top: 6px solid #0f1419;';
  } else if (placement === 'right') {
    top = rect.top + (rect.height / 2) - (cardRect.height / 2);
    left = rect.right + 12;
    // Arrow left
    arrow.style.cssText = 'left: -6px; top: 50%; margin-top: -6px; border-top: 6px solid transparent; border-bottom: 6px solid transparent; border-right: 6px solid #0f1419;';

    // Fallback if offscreen right
    if (left + cardRect.width > window.innerWidth) {
      left = rect.left - cardRect.width - 12;
      // Arrow right
      arrow.style.cssText = 'right: -6px; top: 50%; margin-top: -6px; border-top: 6px solid transparent; border-bottom: 6px solid transparent; border-left: 6px solid #0f1419;';
    }
  }
  ...
  coachmark.style.top = `${top + window.scrollY}px`;
  coachmark.style.left = `${left}px`;

  // Event Listeners
  coachmark.querySelector('[data-action="next"]').onclick = onNext;
  coachmark.querySelector('[data-action="skip"]').onclick = endOnboarding;
}

function cleanupCoachmark() {
  const existing = document.querySelector('.ts-coachmark');
  if (existing) existing.remove();
}

async function endOnboarding() {
  cleanupCoachmark();
  // Remove any stray highlights
  document.querySelectorAll('.ts-onboarding-highlight').forEach(el => el.classList.remove('ts-onboarding-highlight'));
  document.querySelectorAll('.ts-text-blink-cyan').forEach(el => el.classList.remove('ts-text-blink-cyan'));

  // Save completion state
  await chrome.storage.local.set({ [ONBOARDING_KEY_V404]: true });
}

// Step 1: Flags & Location
function showOnboardingStep1(pill) {
  // Target the flag specifically for better arrow alignment
  const flagPart = pill.querySelector('.ts-flag-part') || pill;

  createCoachmark({
    target: flagPart,
    text: 'Flags & Location — instantly see where an account is based.',
    placement: 'top',
    onNext: () => showOnboardingStep2(pill),
    onSkip: endOnboarding
  });
}

// Step 2: Details Link
function showOnboardingStep2(pill) {
  // Transfer highlight
  pill.classList.remove('ts-onboarding-highlight');

  const detailsLink = pill.querySelector('.ts-details-link');
  if (!detailsLink) return endOnboarding();

  // Highlight Text Only (Blink Cyan)
  detailsLink.classList.add('ts-text-blink-cyan');

  // Auto-advance timer (3s)
  const autoAdvanceTimer = setTimeout(() => {
    // If still on screen and not skipped/clicked
    if (document.contains(detailsLink) && document.querySelector('.ts-coachmark')) {
      proceedToDetails();
    }
  }, 3000);
  ...
  const proceedToDetails = () => {
    ...
    detailsLink.classList.remove('ts-text-blink-cyan');
    // Keep it clean
    detailsLink.style.textShadow = '';
    detailsLink.style.color = '';
  ...
  createCoachmark({
    target: detailsLink,
    text: 'Click Details to see account age, verification, device & more.',
    placement: 'bottom',
    onNext: proceedToDetails,
    onSkip: () => {
      clearTimeout(autoAdvanceTimer);
      detailsLink.classList.remove('ts-text-blink-cyan');
      endOnboarding();
...
// Step 3: Wait for Hovercard
function waitForHovercard() {
  cleanupCoachmark(); // Hide while waiting

  let attempts = 0;
  const interval = setInterval(() => {
    attempts++;
    const card = document.querySelector('.ts-hovercard.visible');
    if (card) {
      clearInterval(interval);
      // Wait a tiny bit for render
      setTimeout(() => walkthroughHovercard(card), 400);
    } else if (attempts > 50) { // 5s timeout
      clearInterval(interval);
      endOnboarding(); // Give up
    }
  }, 100);
}

// Step 4: Hovercard Walkthrough (Multi-step)
function walkthroughHovercard(card) {
  // Helper to find rows
  const getRow = (field) => {
    const val = card.querySelector(`[data-field="${field}"]`);
    return val ? val.closest('.ts-hc-row') || val : card;
  };

  const steps = [
    {
      target: getRow('daysOnX'),
      text: 'Account Age & Creation Date — Spot fresh (potentially bot) or recycled accounts.',
      placement: 'right'
    },
    {
      target: getRow('location'), // fallback to 'createdIn' handled below or just points to row
      text: 'Location Analysis — Sees through VPNs/proxies to find the true origin.',
      placement: 'right'
    },
    {
      target: getRow('device'),
      text: 'Device Source — "Twitter for iPhone" vs "Web" vs automated tools.',
      placement: 'right'
    },
    {
      target: getRow('blueVerified'), // Section for verification
      text: 'Verification Check — Breakdown of Blue, ID, and Legacy verification status.',
      placement: 'right'
    }
  ];
  ...
  const showStep = () => {
    ...
    setTimeout(() => {
      if (currentStep === steps.length - 1) {
        const nextBtn = document.querySelector('.ts-coachmark [data-action="next"]');
