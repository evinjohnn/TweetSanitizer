...
async function loadStats() {
  try {
    ...
    // Count valid entries
    const now = Date.now();
    const TTL = 30 * 24 * 60 * 60 * 1000; // 30 days TTL
    let validCount = 0;
    let totalSize = 0;

    for (const [username, data] of Object.entries(cache)) {
      // Check by timestamp (cache stores { timestamp, data })
      if (data.timestamp && (now - data.timestamp < TTL)) {
        validCount++;
        // Rough size calculation - check both data.data.location and data.location
        const location = data.data?.location || data.location || '';
        totalSize += username.length + location.length + 50; // 50 bytes overhead
      }
    }
